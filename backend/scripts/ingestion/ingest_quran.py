...
Populates `surahs`, `ayahs`, and `translations` (English + Urdu). The Arabic
text itself lives in `ayahs.text_ar` — there is no separate 'ar' translation
row, since the source text already is Arabic.
...
from scripts.ingestion.common import db_session, upsert_many, upsert_one
    ...
    for surah in data["surahs"]:
        ...
        for ayah in surah["ayahs"]:
            ayah_id = upsert_one(
                ...
                index_elements=["surah_id", "ayah_number"],
            )
            ayah_id_by_key[(surah["number"], ayah["numberInSurah"])] = ayah_id
            total += 1
    session.commit()
    print(f"Ingested {total} ayahs")
